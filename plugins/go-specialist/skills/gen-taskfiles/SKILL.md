...
4. **Verify skill assets directory:**
   - Use Read tool to verify `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/Taskfile.yml` exists
...
2. **Taskfile.yml** (66 lines)
   - Path: `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/Taskfile.yml`
   - Target: `Taskfile.yml` (project root)

3. **Taskfile_dev.yml** (14 lines)
   - Path: `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/Taskfile_dev.yml`
   - Target: `Taskfile_dev.yml` (project root)

4. **.pre-commit-config.yaml** (25 lines)
   - Path: `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/.pre-commit-config.yaml`
...
**Error handling:**
- If any Read fails: Exit with error "Failed to read template file: {filename}. Plugin may be corrupted. Ensure ${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/ directory exists."

### Phase 3.5: Auto-Detection & Substitution

**Detection Function Library:** Use the shared detection functions from `${CLAUDE_PLUGIN_ROOT}/assets/detection-functions.md`. Read that file to get all bash detection functions (`detect_project_name`, `detect_git_owner`, `detect_main_dir`, `detect_registry`, `detect_go_version`, `detect_golangci_lint_version`, `detect_goreleaser_version`, `strip_leading_v`, `substitute_placeholders`, `substitute_version_placeholders`).
...
- Suggest using the `/gen-linter` or `/gen-goreleaser` skills
...
|-------------|--------------------------|-------|---------|
| `${CLAUDE_PLUGIN_ROOT}/assets/mise/mise.toml` | `mise.toml` | ~35 | Pinned dev tool versions (preserve-if-exists) |
| `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/Taskfile.yml` | `Taskfile.yml` | 66 | Main task definitions |
| `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/Taskfile_dev.yml` | `Taskfile_dev.yml` | 14 | Development tasks |
| `${CLAUDE_PLUGIN_ROOT}/assets/taskfiles/.pre-commit-config.yaml` | `.pre-commit-config.yaml` | 25 | Git pre-commit hooks |
