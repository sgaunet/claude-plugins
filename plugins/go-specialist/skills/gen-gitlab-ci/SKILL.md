...
4. **Verify skill assets directory:**
   - Use Read tool to verify `${CLAUDE_PLUGIN_ROOT}/assets/gitlab-ci/.gitlab-ci.yml` exists
...
1. **.gitlab-ci.yml**
   - Path: `${CLAUDE_PLUGIN_ROOT}/assets/gitlab-ci/.gitlab-ci.yml`
...
**Detection Function Library:** Use the shared detection functions from `${CLAUDE_PLUGIN_ROOT}/assets/detection-functions.md`. Read that file to get the bash detection functions (`detect_go_version`, `detect_golangci_lint_version`, `detect_goreleaser_version`, `strip_leading_v`, `substitute_version_placeholders`).
...
- **Troubleshooting:** "No such runner tags: gitlab-org-docker" → update the runner tags
  ...
  the repo root (this skill creates it). "Docker login failed" (only with the optional
...
- Suggest using the `/gen-goreleaser` skill
...
|-------------|--------------------------|------|
| `${CLAUDE_PLUGIN_ROOT}/assets/gitlab-ci/.gitlab-ci.yml` | `.gitlab-ci.yml` | Always |
...
2. Generate .goreleaser.yml with the `/gen-goreleaser` skill
