...
- **go-oat skill**: Scaffolds the oat.ink HTML+CSS UI library into Go web apps with embedded assets (`//go:embed`) for zero-dependency binaries. Invoke when the user asks for a simple HTML UI without a JS framework, when `oat.min.css`/`oat.min.js` are present, or when an `html/template`/`templ` project needs a styling baseline. Always embeds assets — never references CDN URLs at runtime.
