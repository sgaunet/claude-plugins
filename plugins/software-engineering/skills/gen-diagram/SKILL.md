...
3. **Verify skill assets:**
   - Use Read to confirm `${CLAUDE_PLUGIN_ROOT}/assets/gen-diagram/d2-cheatsheet.md` exists.
   - If missing: Exit with error "Cheatsheet not found at ${CLAUDE_PLUGIN_ROOT}/assets/gen-diagram/d2-cheatsheet.md. Plugin may be corrupted. Reinstall the software-engineering plugin."
...
1. **Read the cheatsheet** to ground the LLM in real d2 syntax and verified icon URLs:
   ```
   Read: ${CLAUDE_PLUGIN_ROOT}/assets/gen-diagram/d2-cheatsheet.md
   ```
...
- **Never rewrite an existing pin.** If `mise.toml` already lists d2 at any version, leave it
  alone and use it — the project's choice wins over this skill's default.
